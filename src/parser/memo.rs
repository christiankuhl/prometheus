...
use super::{Expression, Statement, ParseResult, ParserState};

pub(super) fn try_remember<R>(
    input: ParserState,
    caller: fn(ParserState) -> ParseResult<Rc<R>>,
) -> Option<ParseResult<Rc<R>>> where R: Cacheable {
...
trait Cacheable {
    fn try_load<'a>(cache: &'a RefCell<ParserCache<'a>>, key: (usize, usize)) -> Option<ParseResult<'a, Rc<Self>>>;
    fn store<'a>(cache: &'a RefCell<ParserCache<'a>>, key: (usize, usize), value: &ParseResult<'a, Rc<Self>>);
}
